...
func (ep *endpoint) AddStaticRoute(destination *net.IPNet, routeType int, nextHop net.IP, src net.IP) error {
	ep.Lock()
	defer ep.Unlock()

	r := types.StaticRoute{Destination: destination, RouteType: routeType, NextHop: nextHop, Src: src}
